...
for sp in *.fa
	...
	#kallisto index -k 21 -i $sp.21.idx $sp
    ...
    kallisto quant -i ${pathExonFasta}/$sp.21.idx -o ${pathResults}/$i.21 -b 100 --single -l 76 -s 1 RNAseq.fastq
    fi
	
	if [ ${numreads} = "2" ]; then
    echo paired ends reads
    kallisto quant -i ${pathExonFasta}/$sp.21.idx -o ${pathResults}/$i.21 -b 100 RNAseq_1.fastq RNAseq_2.fastq
    fi
