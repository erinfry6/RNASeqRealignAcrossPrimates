...
```{r set directories and load libraries, include=FALSE, warning=FALSE}
...
TPM_noMT_PCA_expressed<- TPM_noMT_PCA[rowSums(TPM_noMT_PCA)>2*ncol(TPM_noMT_PCA),]
...
```{r load sampleinfo and eliminate bad samples}
...
## eliminate hsa_br_M_ 4 and 5 from SampleInfo and TPM_noMT_PCA_expressed_sqrt
SampleInfo<-SampleInfo[-c(7,8),]
TPM_noMT_PCA_expressed_sqrt_quality<-TPM_noMT_PCA_expressed_sqrt[,-c(7,8)]
...
dge<-DGEList(counts=TPM_noMT_PCA_expressed_sqrt_quality,lib.size=colSums(TPM_noMT_PCA_expressed_sqrt_quality))
...
summary(colnames(TPM_noMT_PCA_expressed_sqrt_quality) %in% colnames(dge))
...
TPM.voom.cyclic<-voom(dge,design = model.matrix(~0 + SampleInfo$Species),normalize.method = "cyclicloess", plot=T)
