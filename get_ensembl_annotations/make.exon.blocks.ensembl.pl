...
sub makeExonBlocks{
	...
	if($chr eq "NA"){
	    print $gene." is protein-coding but does not have protein-coding transcripts!!\n";
	}
	else{
...
my %defaultvalues;
my @defaultpars=("pathExonCoords","pathExonAssignment","pathGeneInfo","pathTranscriptInfo","collapseDistance","filter","pathOutputExonBlocks");
...
print "filter ".$filter."\n";
makeExonBlocks(\%genetranscripts,\%transcriptexons,\%genebio, \%txbio,\%exoncoords,$parameters{"collapseDistance"},$filter,\%exonblocks);
print "Done.\n\n";
